...
const TCHAR LibUIDK::c_szTreeViewClass[] = _T("SysTreeCtrl32");
...
BOOL FAR TV_Init(HINSTANCE hinst)
{
	...
	if (!GetClassInfo(hinst, c_szTreeViewClass, &wc))
	{
		...
		wc.cbWndExtra      = sizeof(PTREE); // 这样，SetWindowLong(HWND, 0, void *)才能设置正确，0一般用来存放控件变量
		wc.cbClsExtra      = 0;
...
BOOL TV_CheckForDragBegin(PTREE pTree, HTREEITEM hItem, int x, int y)
{
	...
	else
	{
		...
		_ASSERT(hItem == pTree->hDropTarget);
	}
...
BOOL TV_EndWatch(PTREE pTree, PTVWATCHEDITEM pwi)
{
	...
	while (--i >= 0)
	{
		PTVWATCHEDITEM pwiT = (PTVWATCHEDITEM)DPA_FastGetPtr(pTree->hdpaWatch, i);
		_ASSERT(pwiT);
		if (pwi == pwiT)
		{
			IUI_DPA_DeletePtr(pTree->hdpaWatch, i);
			return TRUE;
		}
	}
