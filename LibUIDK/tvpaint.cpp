...
void NEAR TV_CreateIndentBmps(PTREE pTree)
{
	...
	hBmpOld = pTree->hBmp;
	pTree->hBmp = NULL;//L CreateColorBitmap(cnt * pTree->cxIndent, pTree->cyItem);
	if (hBmpOld)
	{
	...
	hbrOld = (HBRUSH)SelectObject(hdc, hbrLine);
...
void NEAR TV_DrawTree(PTREE pTree, HDC hdc, BOOL fErase, LPRECT lprc)
{
	...
	RECT rc = *lprc; // Tree大小，包含非客户区（滚动条）
	...
	if (!(pTree->ci.dwCustom & CDRF_SKIPDEFAULT))
	{
		...
		if (iStart < iCnt)
		{
			...
			// TVDI_* for all items
			UINT uFlags = (pTree->ci.style & WS_DISABLED) ? TVDI_GRAYCTL : 0;
			if (fErase)
			{
				uFlags |= TVDI_ERASE;
			}
...
HIMAGELIST NEAR TV_CreateDragImage(PTREE pTree, HTREEITEM hItem)
{
	...
	BOOL bMirroredWnd = FALSE;//L (pTree->ci.dwExStyle & RTL_MIRRORED_WINDOW);
	int dx, dy;
	int iSrc;
	...
	*/
	...
	//	pTree->hImageList, iSrc, 0L | (hItem->state & TVIS_OVERLAYMASK));
...
LRESULT TV_GenerateDragImage(PTREE pTree, SHDRAGIMAGE *pshdi)
{
	...
	//
	//L if (pTree->ci.dwExStyle & RTL_MIRRORED_WINDOW)
	//{
